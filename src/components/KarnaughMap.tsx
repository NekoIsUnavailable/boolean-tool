    ...
    <div className="modern-card p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
           ...
           className="text-sm px-3 py-1.5 bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-200 text-slate-700 dark:text-slate-300 rounded-md flex items-center gap-2 transition-colors"
          ...
          <thead>
            {showTopQuadrants && (
              <tr>
                ...
                <th colSpan={4} className="text-center font-bold text-slate-800 dark:text-slate-200 pb-2 text-lg">
            ...
            <tr>
              ...
              <th className="p-2 text-slate-500 dark:text-slate-400 font-medium text-sm text-right align-bottom border-b-2 border-r-2 border-slate-500">
                {cornerLabel}
              </th>
              {Array(numCols).fill(0).map((_, i) => (
                <React.Fragment key={i}>
                  <th className="p-2 text-center text-slate-700 dark:text-slate-300 font-mono text-sm border-b-2 border-slate-500">
                    {innerColLabels[i % 4]}
                  </th>
              ...
              <React.Fragment key={rIdx}>
                <tr>
                  {showSideQuadrants && rIdx === 0 && (
                    <th rowSpan={4} className="px-2 text-center font-bold text-slate-800 dark:text-slate-200 text-lg">
                  ...
                  <th className="p-2 text-right text-slate-700 dark:text-slate-300 font-mono text-sm border-r-2 border-slate-500 align-middle">
                  ...
                  {row.map((minterm, cIdx) => {
                    ...
                    return (
                      <React.Fragment key={cIdx}>
                        <td 
                          className={cn(
                            ...
                            state === '1' ? 'bg-indigo-50' : state === 'X' ? 'bg-slate-50 dark:bg-slate-900' : 'bg-white',
                      ...
                      <button
                        onClick={() => onChange(minterm, cycleState(state))}
                        className="absolute inset-0 w-full h-full flex items-center justify-center font-bold text-xl z-10 transition-colors hover:bg-slate-50 dark:bg-slate-900/50"
                      >
