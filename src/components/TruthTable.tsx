    ...
    <div className="modern-card p-6">
      ...
      <div className="overflow-x-auto max-h-[400px] overflow-y-auto">
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-slate-700 dark:text-slate-300 uppercase bg-slate-50 dark:bg-slate-900 sticky top-0 z-10">
            <tr>
              <th className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">m</th>
              {varNames.map((name, i) => (
                <th key={i} className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 text-center">
                  {name}
                </th>
              ))}
              <th className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 text-center border-l-2 border-slate-300">Out</th>
            </tr>
          </thead>
          <tbody>
            {Array.from({ length: rows }).map((_, rowIndex) => {
              ...
              return (
                <tr key={rowIndex} className="border-b border-slate-100 hover:bg-slate-50 dark:bg-slate-900 transition-colors">
                  <td className="px-4 py-2 font-mono text-slate-500 dark:text-slate-400">{rowIndex}</td>
                  ...
                  <td className="px-4 py-2 text-center border-l-2 border-slate-300">
                    <button
                      ...
                      className={cn(
                        ...
                        outState === '0' ? "bg-slate-200 text-slate-700 dark:text-slate-300 hover:bg-slate-300" :
